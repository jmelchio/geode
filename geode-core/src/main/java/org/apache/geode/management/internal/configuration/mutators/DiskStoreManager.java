...
public class DiskStoreManager extends CacheConfigurationManager<DiskStore> {
  ...
  public List<DiskStore> list(DiskStore filterConfig, CacheConfig existing) {
    return existing.getDiskStores().stream().map(diskStoreConverter::fromXmlObject).collect(
        Collectors.toList());
  }

  @Override
  public DiskStore get(DiskStore config, CacheConfig existing) {
    return existing.getDiskStores().stream().filter(diskStoreType -> diskStoreType.getName().equals(config.getName())).map(
        diskStoreConverter::fromXmlObject).findFirst().orElse(null);
  }
}
